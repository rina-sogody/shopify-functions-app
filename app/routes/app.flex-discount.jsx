  ...
  return (value || [])
    .map((entry) => {
      if (typeof entry === "string") {
        return entry.trim() ? { sku: entry.trim(), variantId: null, productId: null, title: "", productTitle: "", image: null } : null;
      }
      if (!entry || typeof entry !== "object") return null;
      const sku = entry.sku?.trim() || null;
      const variantId = entry.variantId || entry.id || null;
      if (!sku && !variantId) return null;
      return { sku, variantId, productId: entry.productId || null, title: entry.title || "", productTitle: entry.productTitle || "", image: entry.image || null };
    })
    .filter(Boolean);
}
...
export default function FlexDiscountPage() {
  ...
  const [settings, setSettings] = useState(() => {
    const data = status?.hydratedSettings || safelyParseJson(status?.metafield?.value);
    return data ? { tiers: normalizeTiers(data.tiers), eligibleSkus: normalizeEligibleSkus(data.eligibleSkus) } : { tiers: [], eligibleSkus: [] };
  });

  const { loading, setBanner, create, save, toggleStatus, remove } = useDiscount({ type: "flex", navigate, discountId });

  const sortedTiers = useMemo(() => [...settings.tiers].sort((a, b) => a.threshold - b.threshold), [settings.tiers]);
  ...
  const bannerError = (msg) => setBanner({ message: msg, tone: "critical" });

  function validate() {
    if (!title?.trim()) return bannerError("Discount name is required"), false;
    if (!settings.tiers.length) return bannerError("Add at least one tier"), false;
    const seen = new Set();
    for (const t of settings.tiers) {
      ...
      if (seen.has(t.threshold)) return bannerError("Thresholds must be unique"), false;
  ...
  const handleCreate = () => validate() && create({ title, settings: getSortedSettings() });
  const handleSave = () =>
    validate() &&
    save({
      title,
  ...
  const handleToggle = (s) => { toggleStatus({ settings: getSortedSettings(), newStatus: s }); setIsActive(s === "ACTIVE"); };

  const addTier = () => {
    const last = sortedTiers[sortedTiers.length - 1];
    setSettings((p) => ({ ...p, tiers: [...p.tiers, { threshold: last ? last.threshold + 5000 : 5000, percent: last ? Math.min(last.percent + 5, 50) : 10 }] }));
  };

  const removeTier = (i) => setSettings((p) => ({ ...p, tiers: p.tiers.filter((_, idx) => idx !== i) }));

  const updateTier = (i, patch) => setSettings((p) => ({ ...p, tiers: p.tiers.map((t, idx) => idx === i ? { ...t, ...patch } : t) }));
    ...
    <s-page heading={isEdit ? "Edit tiered discount" : "Create tiered discount"}>
      ...
      {isEdit && (
        <>
          ...
          <s-button slot="secondary-actions" tone="critical" onClick={remove} disabled={loading}>Delete</s-button>
        </>
      )}
      ...
      <s-section heading="Products">
        <VariantSkuPicker
          ...
          disabled={loading}
          onChange={(skus) => setSettings((p) => ({ ...p, eligibleSkus: skus }))}
      ...
      <s-section heading="Tiers">
        {settings.tiers.length === 0 ? (
          <s-stack gap="base">
            <s-paragraph>No tiers yet. Add spending thresholds with discounts.</s-paragraph>
            <s-button variant="primary" onClick={addTier} disabled={loading}>Add tier</s-button>
          </s-stack>
        ) : (
          <s-stack gap="base">
            <s-table>
              ...
              <s-table-body>
                {settings.tiers.map((tier, i) => (
                  <s-table-row key={i}>
                    ...
                    <s-table-cell>
                      <s-button size="slim" tone="critical" onClick={() => removeTier(i)} disabled={loading}>×</s-button>
                    </s-table-cell>
                  </s-table-row>
                ))}
              </s-table-body>
            </s-table>
            <s-button onClick={addTier} disabled={loading}>Add tier</s-button>
          </s-stack>
        )}
      </s-section>
      ...
      {sortedTiers.length > 0 && (
        <s-section heading="Summary">
          <s-paragraph>
            {sortedTiers.map((t) => `€${(t.threshold / 100).toFixed(0)}+ = ${t.percent}% off`).join(" • ")}
          </s-paragraph>
        </s-section>
      )}
    </s-page>
